...
public class BST<E extends Comparable<E>> {
        ...
        if (root == null) // the first node to be inserted
            ...
            while (node != null) { // Looking for a leaf node
                ...
                if(value.compareTo(node.value) < 0) {
                    ...
                    System.out.println("Duplicates: " + node.value + ", " + value);
    ...
    */
    ...
    /**
        ...
        @return value of the node that matches the value searched
        Time complexity: o(n)
     */
    public E get(E value){
        /*The method get(E item), in the class BST, 
        returns the value of the node that is equal to item or null 
        if not found. The method must be recursive. 
    */
        //USE ARRAYLIST
        int countIndex = 0;
        ArrayList<E> list = new ArrayList<>();
        list = listing(list, root);
        int indexFound = get(value, list, countIndex);

        return list.get(indexFound); //CHECK
    }
    public int get(E value, ArrayList<E> list, int i){
        //search in here 
        if (!list.contains(value)){
            return -1; //might have to do -1 for index
    ...
    public ArrayList<E> listing(ArrayList<E> list, TreeNode node){
        //fill it here
        if (node != null) {
            inorder(node.left);
            list.add(node.value); //using inorder - sorted
            inorder(node.right);
        }
        return list;
    ...
    private void values(TreeNode node, ArrayList<E> list){
        if(node != null){
            list.add(node.value);
